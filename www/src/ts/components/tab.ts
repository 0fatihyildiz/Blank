export function handleTabButtonClick(button: HTMLButtonElement) {
...
function desactivateAll(tabButtons: NodeListOf<HTMLElement>, tabPanes: NodeListOf<HTMLElement>) {
    tabButtons.forEach(button => button.removeAttribute('data-active'))
    tabPanes.forEach(pane => pane.removeAttribute('data-active'))
}

function activateTab(button: HTMLButtonElement, tabButtons: NodeListOf<HTMLElement>, tabPanes: NodeListOf<HTMLElement>) {
    const index = Array.from(tabButtons).indexOf(button)

    button.setAttribute('data-active', 'true')
    if (tabPanes[index])
        tabPanes[index].setAttribute('data-active', 'true')
}
